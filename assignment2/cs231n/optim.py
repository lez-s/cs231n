...
def sgd_momentum(w, dw, config=None):
    ...
    v = config.get("velocity", np.zeros_like(w)) #字典get()函数返回key的value，不存在就返回默认
    ...
    v=momentum*v-learning_rate*dw #课件上的
    next_w=w+v #这个地方有个小失误，原来的w加上v才是下一个w
...
def adam(w, dw, config=None):
    ...
    t = config.setdefault("t", 0) #如果没有值就设为0
    ...
    t +=1 #资料上写的是迭代counter。这个地方要注意，因为自己没写出来。
    #print(t)
