...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        如果y=None，就返回scores，否则返回loss和grad
        ...
        """
        # Unpack variables from the params dictionary 获取初始化参数
        ...
        #f = lambda x: 1.0/(1.0 + np.exp(-x)) # 激活函数(⽤的sigmoid) 从note拷贝来的
        #OK，好吧，以上实际上是用不到的。
        h1=np.dot(X,W1)+b1  
        h2=h1
        h2[h2<0]=0        
        ...
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        #以下为softmax函数
        scores = scores - np.max(scores,axis=1).reshape(-1,1) #平移防止数据爆炸

        sum_e=np.sum(np.exp(scores),axis=1).reshape(-1,1)
        scores_pob=np.exp(scores)/sum_e
        scores_pob_right=scores_pob[np.arange(num_train),y]
        loss=np.sum(-np.log(scores_pob_right))/num_train
